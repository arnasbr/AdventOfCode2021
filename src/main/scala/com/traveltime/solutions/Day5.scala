    ...
    val allPoints = for {
      ...
      allPoints = filteredVents.flatMap { vent =>
        if (vent.start.x == vent.end.x) {
          val first = vent.start.y
          val last = vent.end.y
          Range
            .inclusive(first min last, first max last)
    ...
    val allPoints = for {
      ...
      allPoints = vents.flatMap { vent =>
        if (vent.start.x == vent.end.x && vent.start.y != vent.end.y) {
          ...
          val xFirst = vent.start.x
          val xLast = vent.end.x
          val yFirst = vent.start.y
          val yLast = vent.end.y

          val range1 =
            if (xFirst <= xLast)
              Range.inclusive(xFirst min xLast, xFirst max xLast)
            else Range.inclusive(xFirst min xLast, xFirst max xLast).reverse

          val range2 =
            if (yFirst <= yLast)
              Range.inclusive(yFirst min yLast, yFirst max yLast)
            else Range.inclusive(yFirst min yLast, yFirst max yLast).reverse
